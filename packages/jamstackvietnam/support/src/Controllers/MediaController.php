...
class MediaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($files as $file) {
            $fileUploaded[] = $this->model::storeFile($file, $uploader);
        }
    ...
    public function cache($staticId, $year, $month, $day, $filename)
    {
        try {
            $disk = $staticId === 'guest' ? 'guest' : 'uploads';
            $fullPath = $year . '/' . $month . '/' . $day . '/' . $filename;
            $extension = $this->getExtension($filename);
            if ($extension['is_image']) {
                $data = request()->all();
                $data['fm'] = request()->input('fm', 'webp');

                $server = ServerFactory::create([
                    'response' => new LaravelResponseFactory(),
                    'source' => config("filesystems.disks.$disk.root"),
                    'cache' => config("filesystems.disks.$disk.root") . "/cache",
                    'max_image_size' => 1600 * 1600
                ]);
                return $server->getImageResponse($fullPath, $data);
            }

            $fullPath = config("filesystems.disks.$disk.root") . '/' . $fullPath;
            ...
            if ($extension['is_svg']) {
                ...
                $media = $this->model::where('id', $staticId)->firstOrFail();
                $mine = $media->mime;
            }
