...
if (!function_exists('shipping_cost')) {
    function shipping_cost($sub_total)
    {
        if (intval($sub_total) > 0 && intval($sub_total) < 1000000) {
            return 25000;
        }
        return 0;
    }
...
if (!function_exists('form')) {
    function form($model)
    {
        $keys = array_keys(array_flip($model->fillable));
        $fillable = array_fill_keys($keys, null);

        $rules = $model->modelRules();
        return [
            'fillable' => $fillable,
            'rules' => $rules['store'] ?? $rules['all'],
        ];
    }
}
