<template>
  ...
  <template v-else-if="field.type === 'money'">
    <div class="relative group">
      ...
      />
      <label
        class="absolute top-0 block w-full h-full bg-white input group-hover:hidden"
      >
        {{ toMoney(modelValue) }}
      </label>
    </div>
  ...
  <template v-else-if="field.type === 'select_single'">
    <select
      ...
      :class="{
        'text-gray-600':
          !modelValue || (modelValue.length === 0 && field.placeholder),
      }"
      :readonly="field.readonly ?? false"
      :disabled="isDisabled"
      @input="
    ...
    >
      ...
      <option
        ...
        :key="id"
        :value="item[keyBy] !== undefined ? item[keyBy] : id"
  ...
  <template v-else-if="field.type === 'toggle'">
    <div class="flex items-center space-x-4">
      <label
        class="relative inline-block w-12 h-6 overflow-hidden outline-none cursor-pointer toggle ring-transparent"
      >
        <input
          type="checkbox"
          :checked="modelValue"
      ...
      <div>
        <div class="checkbox">
          <input
            ...
            :value="item.id"
            :indeterminate.prop="
              containsSome(modelValue, pluck(item.items ?? []))
            "
            v-model="modelValue"
...
export default {
  ...
  computed: {
    isDisabled() {
      return this.disabled ?? this.field.readonly ?? false;
    },
    fieldId() {
  ...
  created() {
    if (this.modelValue) {
      ...
      (this.field.type === "select_single" || this.field.type === "radio") &&
  ...
  methods: {
    ...
    optionSelected(item, id) {
      const value = item[this.keyBy] !== undefined ? item[this.keyBy] : id;
      const modelValue =
        typeof this.modelValue === "object"
          ? this.modelValue[this.keyBy]
          : this.modelValue;
      return value.toString() === (modelValue ? modelValue.toString() : "");
    },
