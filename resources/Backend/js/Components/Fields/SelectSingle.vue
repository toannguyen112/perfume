...
export default {
  ...
  computed: {
    isSelectSourceSimple() {
      return (
        this.field.config.source !== undefined &&
        this.field.config.source.type === "simple"
      );
    },
    isSelectSourceDirectory() {
      return (
        this.field.config.source !== undefined &&
        this.field.config.source.type === "directory"
      );
    },
    selectLabelField() {
    ...
    selectedOptions() {
      ...
      if (this.field.config.getObject || this.isSelectSourceSimple)
        return this.value;
      if (this.field.config.options === undefined) return [];

      return this.field.config.options.find(
        (x) => x[this.selectTrackByField] === this.value
      );
    },
  },
