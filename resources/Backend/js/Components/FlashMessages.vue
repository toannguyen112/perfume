<template>
  <div
    v-if="flash.success"
    class="flex items-center justify-between mb-6 text-white bg-green-500 rounded"
  >
  ...
  <details
    v-else-if="flash.error && !hasErrors"
    class="py-4 mb-6 text-sm text-white bg-red-400 rounded"
  >
    <summary class="items-center justify-between px-3 font-medium flex-inline">
      ...
      <button
        ...
        @click="
          ($page.props.flash.error = null), ($page.props.errors = [])
        "
      >
        <Icon name="close" />
      </button>
    </summary>
    <ul v-if="hasErrors" class="px-6 mt-2 list-disc list-inside">
      <li v-for="(error, key) in errors" :key="key">{{ error }}</li>
    </ul>
  </details>
</template>
