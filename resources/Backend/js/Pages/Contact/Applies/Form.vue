    ...
    <div class="space-y-3 card">
      <template v-for="(key, field) in form.data_contact" :key="field">
        <div v-if="typeof form.data_contact[field] === 'object'">
          <div v-if="field == 'Job'" class="font-bold">
            <Link
                :href="
                  route('sidebar.jobs.form', {
                    id: form.data_contact[field]['id'],
                  })
                "
                target="_blank"
                class="link"
                >Job: {{ form.data_contact[field]['title'] }}</Link
              >
          </div>
          <div v-else class="font-bold">{{ field }}:</div>
          <a :href="form.data_contact[field]['Path']" target="_blank">
            {{ form.data_contact[field]["Path"] }}
          </a>
        </div>
        <div v-else>
          <Fieldset
