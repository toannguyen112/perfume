<template>
  <section class="mb-4 product-new md:mb-8" v-if="list && list.length > 0">
    <div class="container">
      <h2 class="xl:mb-[24px] md:mb-[17px] mb-[12px] p2">
        <span class="font-bold text-black">SẢN PHẨM MỚI</span>
      </h2>
      <div class="relative slider -mx-0.5 sm:-mx-2.5">
        ...
        >
          <template v-for="(item, index) of list.slice(0, 5)" :key="index">
            <swiper-slide class="productNewCard">
              <ProductCard
                :item="item"
                class="h-full"
...
export default {
  ...
  methods: {
    ...
    setHeightCard() {
      const items = document.querySelectorAll(".productNewCard");
      const heights = [];
